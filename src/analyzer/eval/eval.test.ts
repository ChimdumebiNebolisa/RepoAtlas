...
describe("analyzer evaluation suite (fixture gold labels)", () => {
  ...
  it("loads gold labels for the seeded fixtures", () => {
    expect(goldLabels.map((item) => item.fixture).sort()).toEqual([
      ...
      "repo-node-api",
      "repo-python",
      "repo-python-import-aliases",
      "repo-ts",
    ]);
  });

  it("matches the human-labeled alias-heavy Python fixture exactly", async () => {
    const gold = goldLabels.find((item) => item.fixture === "repo-python-import-aliases");
  ...
  it("detects every human-labeled monorepo entrypoint without extras", async () => {
    const gold = goldLabels.find((item) => item.fixture === "repo-monorepo");
    expect(gold).toBeDefined();
