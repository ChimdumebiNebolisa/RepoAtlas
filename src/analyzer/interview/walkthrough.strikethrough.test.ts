...
describe("GFM walkthrough strikethrough", () => {
  it.each([
    ["a spaced phrase", "A".repeat(64), "~~repository guide~~"],
    ["a short phrase", "A".repeat(67), "~~setup notes~~"],
    [
      "a hyphenated phrase",
      "A".repeat(61),
      "~~risk-parser review~~",
    ...
    "does not split strikethrough near %s",
    (_, prefix, strikethrough) => {
      const purpose = `${prefix} ${strikethrough} for repository interviews`;
      ...
      expect(purpose).toContain(strikethrough);
    }
  );
});
