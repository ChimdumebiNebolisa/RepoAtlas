...
3. **Retention is server-side.** `deleteReport()` exists only for internal TTL/max-count sweeps invoked by `GET` or `POST /api/cron/cleanup`.
...
- Vercel schedules the authenticated cleanup `GET` daily; other production operators must schedule the route and set `CRON_SECRET`.
