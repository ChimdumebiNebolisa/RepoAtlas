...
RepoAtlas is a single Next.js 16 (App Router) + React 19 + TypeScript app that performs deterministic, no-AI static analysis of an uploaded repository ZIP or canonical public GitHub URL and produces an evidence-backed Candidate Brief. See `README.md` for the full product overview, API reference, and standard commands.
...
- **Playwright browsers:** e2e needs Chromium and WebKit installed via `npx playwright install --with-deps chromium webkit` (handled by the startup update script).
...
- **Core workflow to smoke-test:** upload a ZIP or public GitHub URL through the UI, click **Run bundled sample**, or POST `{"sample":true}` to `/api/analyze`, then `GET /api/reports/<id>`. Caller-controlled JSON `zipRef` is intentionally rejected by the public API.
